...
Пример фазового портрета для системы Рёсслера (по статье):
...
## Нужно ли отдельно описывать каждую переменную?

Я оставил **общее описание метода и структуры**, а также указал, где лежат результаты для каждой переменной (`X/`, `Y/`, `Z/`, `Q/`, `W/`). Это компромисс между полнотой и читаемостью: при необходимости можно быстро перейти к конкретной переменной и её логам/графикам.

Если нужно, можно расширить README отдельными мини‑подразделами по каждой переменной (например, выделить отличия по `h/WT/K`, устойчивость и форму аттрактора).
