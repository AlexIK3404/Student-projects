%% Hyper-Chen — FB phase map (y-only) + GRID SEARCH over (h, WT, Ky)
% System:
%   x' = a (y - x)
%   y' = -x z + d x + c y - q
%   z' = x y - b z
...
%
% Outputs:
%   - Finds best (h, WT, Ky) for y-only coupling by maximizing "strong capture" fraction
...
itrs_full   = 3000; % FB iterations for final full map (quality)
...
WT_list = [0.08 0.10 0.12 0.15 0.18 0.22 0.25];
...
for ih = 1:numel(h_list)
    ...
    for WT = WT_list
        ...
        for Ky = Ky_list
            Kf = [0; Ky; 0; 0];
            Kb = Kf;  % start symmetric for y-only
...
caxis([-8 1]); grid on; view(3)
title(sprintf('FB map (score\\_min), h=%.4f WT=%.3f Ky=%.2f', h, WT, Ky))
...
xlabel('x'); ylabel('y'); zlabel('z');
colorbar; colormap(turbo);
caxis([-4 1]); grid on; view(3)
title(sprintf('FB map (score\\_end), h=%.4f WT=%.3f Ky=%.2f', h, WT, Ky))
