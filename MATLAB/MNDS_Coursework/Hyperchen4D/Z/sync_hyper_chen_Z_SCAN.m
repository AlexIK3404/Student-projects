%% Hyper-Chen — FB phase map (z-only) + GRID SEARCH over (h, WT, Kz)
% System:
%   x' = a (y - x)
%   y' = -x z + d x + c y - q
%   z' = x y - b z
...
%
% Outputs:
%   - Finds best (h, WT, Kz) for z-only coupling by maximizing "strong capture" fraction
...
%   - score_end  = log10(rms_end) - log10(rms_1)        (strict)
...
Kz_list = [2 5 6.5 10 15 20 30 40 60 80 100];

% Threshold for "strong capture" in score_min
strong_thr = -3;   % score_min < -3 => at least 10^3 reduction at some iteration
    ...
    for WT = WT_list
        ...
        for Kz = Kz_list
            ...
            if metric > best.metric
                ...
                best.medMin = medMin;
                fprintf('BEST: h=%.4f WT=%.3f Kz=%.1f | strong=%.2f medMin=%.2f medEnd=%.2f endNeg=%.2f\n', ...
                    h,WT,Kz,fracStrong,medMin,medEnd,fracEndNeg);
            end
        end
    end
end

fprintf('\nBest found: h=%.4f WT=%.3f Kz=%.1f | strong=%.2f medMin=%.2f\n', ...
    best.h, best.WT, best.Kz, best.fracStrong, best.medMin);
...
title(sprintf('FB map (score\\_min), h=%.4f WT=%.3f Kz=%.2f', h, WT, Kz))
...
colorbar; colormap(turbo);
caxis(CAX); grid on; view(3)
title(sprintf('FB map (score\\_end), h=%.4f WT=%.3f Kz=%.2f', h, WT, Kz))
