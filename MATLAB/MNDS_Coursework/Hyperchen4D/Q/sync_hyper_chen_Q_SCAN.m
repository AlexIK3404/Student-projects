%% Hyper-Chen — FB phase map (q-only) + GRID SEARCH over (h, WT, Kq)
% System:
%   x' = a (y - x)
%   y' = -x z + d x + c y - q
%   z' = x y - b z
...
%
% Outputs:
%   - Finds best (h, WT, Kq) for q-only coupling by maximizing "strong capture" fraction
%   - Plots full phase maps for score_min and score_end using the best params
%
% Notes:
%   - score_min  = min_i [log10(rms_i) - log10(rms_1)]
%   - score_end  = log10(rms_end) - log10(rms_1)
...
%% ================= GRID SEARCH =================
best.metric = -Inf;
best.h = NaN; best.WT = NaN; best.Kq = NaN;
best.fracStrong = NaN; best.medMin = NaN;

results = []; % [h WT Kq fracStrong medMin medEnd fracEndNeg]

fprintf('Grid search: %d h * %d WT * %d Kq = %d configs\n', ...
    numel(h_list), numel(WT_list), numel(Kq_list), numel(h_list)*numel(WT_list)*numel(Kq_list));
...
for ih = 1:numel(h_list)
    ...
    for WT = WT_list
        ...
        for Kq = Kq_list
            ...
            results(end+1,:) = [h WT Kq fracStrong medMin medEnd fracEndNeg]; %#ok<SAGROW>

            % Metric: prioritize many strongly-captured points, then better median min-score
            metric = fracStrong*10 + (-medMin);

            if metric > best.metric
                best.metric = metric;
                best.h = h; best.WT = WT; best.Kq = Kq;
                best.fracStrong = fracStrong;
                best.medMin = medMin;
                fprintf('BEST: h=%.4f WT=%.3f Kq=%.1f | strong=%.2f medMin=%.2f medEnd=%.2f endNeg=%.2f\n', ...
                    h,WT,Kq,fracStrong,medMin,medEnd,fracEndNeg);
            end
        end
    end
end

fprintf('\nBest found: h=%.4f WT=%.3f Kq=%.1f | strong=%.2f medMin=%.2f\n', ...
    best.h, best.WT, best.Kq, best.fracStrong, best.medMin);
...
title(sprintf('FB map (score\\_min), h=%.4f WT=%.3f Kq=%.2f', h, WT, Kq))
...
colorbar; colormap(turbo);
caxis(CAX); grid on; view(3)
title(sprintf('FB map (score\\_end), h=%.4f WT=%.3f Kq=%.2f', h, WT, Kq))

%% ================= SHOW TOP CONFIGS =================
[~,ord] = sort(results(:,5), 'ascend'); % sort by medMin (more negative is better)
topN = min(15, size(results,1));
disp('Top configs sorted by medMin: [h WT Kq fracStrong medMin medEnd fracEndNeg]');
