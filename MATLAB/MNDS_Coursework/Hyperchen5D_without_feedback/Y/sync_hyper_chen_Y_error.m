%% Hyper-Chen 5D — FB error vs iteration (TIME-SHIFT IC, with Option B) — y-only coupling
...
% 5D extra param (only used inside step)
e = 1.0;
...
Xs = Xc;
for i=1:Nshift
    Xs = hyperchen5_step(Xs,a,b,c,d,g,e,h,s,zeros(5,1),zeros(5,1)); % free run, no coupling
end
...
for it = 1:itrs
    ...
    for j = 1:(WT_iter-1)
        ...
        Xs = hyperchen5_step(Xs,a,b,c,d,g,e, h,s, W(:,j), Kf);
    end
    ...
    for j = 1:(WT_iter-1)
        Xs = hyperchen5_step(Xs,a,b,c,d,g,e, -h,s, Wb(:,j), -Kb);
    end
end
...
title(sprintf('Hyper-Chen 5D FB error vs iteration (WT=%.3f, h=%.4f, Ky=%.1f, tau=%.2f)', ...
    WT, h, Ky, tau_shift))
hold off
...
% 5th equation (placeholder 5D mod; swap later if you want)
% w' = -e*w + x*z
w = w + h1*( -e*w + x*z + N(5) );
