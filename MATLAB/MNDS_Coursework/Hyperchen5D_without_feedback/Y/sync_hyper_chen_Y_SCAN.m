%% Hyper-Chen 5D — FB phase map (y-only) + GRID SEARCH over (h, WT, Ky)
% 5D MODIFICATION:
%   x' = a*(y - x)
%   y' = -x*z + d*x + c*y - q
%   z' = x*y - b*z
...
% ===== 5D Hyper-Chen parameters (4D + extra e) =====
a = 36;
b = 3;
c = 28;
d = 16;
...
%% ================= GRID TO SEARCH (y-only coupling) =================
h_list  = [0.0025 0.005];
WT_list = [0.15 0.18 0.22 0.25 0.28 0.30 0.35 0.40];
Ky_list = [2 5 6.5 10 15 20 30 40 60 80 100];
...
%% ================= GRID SEARCH =================
best.metric = -Inf;
best.h = NaN; best.WT = NaN; best.Ky = NaN;
best.fracStrong = NaN; best.medMin = NaN;

results = []; % [h WT Ky fracStrong medMin medEnd fracEndNeg]

fprintf('Grid search: %d h * %d WT * %d Ky = %d configs\n', ...
    numel(h_list), numel(WT_list), numel(Ky_list), numel(h_list)*numel(WT_list)*numel(Ky_list));
...
for ih = 1:numel(h_list)
    ...
    for WT = WT_list
        ...
        for Ky = Ky_list
            ...
            Kforward  = [0; Ky; 0; 0; 0];
            Kbackward = Kforward;  % keep equal (как в твоём базовом Чене)
            ...
            for t = 1:numel(idx)
                ...
                for it = 1:itrs_search
                    ...
                    for j = 1:(WT_iter-1)
                        ...
                        Xs = hyperchen5_step(Xs,a,b,c,d,g,e, h,s, WT_forward(:,j), Kforward);
                    end
                    % backward (paper style: -h and -Kbackward)
                    for j = 1:(WT_iter-1)
                        Xs = hyperchen5_step(Xs,a,b,c,d,g,e, -h,s, WT_backward(:,j), -Kbackward);
                    end
            ...
            results(end+1,:) = [h WT Ky fracStrong medMin medEnd fracEndNeg]; %#ok<SAGROW>

            metric = fracStrong*10 + (-medMin);
            if metric > best.metric
                best.metric = metric;
                best.h = h; best.WT = WT; best.Ky = Ky;
                best.fracStrong = fracStrong;
                best.medMin = medMin;
                fprintf('BEST: h=%.4f WT=%.3f Ky=%.1f | strong=%.2f medMin=%.2f medEnd=%.2f endNeg=%.2f\n', ...
                    h,WT,Ky,fracStrong,medMin,medEnd,fracEndNeg);
            end
        end
    end
end

fprintf('\nBest found: h=%.4f WT=%.3f Ky=%.1f | strong=%.2f medMin=%.2f\n', ...
    best.h, best.WT, best.Ky, best.fracStrong, best.medMin);
...
hw = waitbar(0,'Computing full phase maps (5D time-shift IC)...');
...
%% ================= PLOTS (3D phase, coloring from scores) =================
% plot (x,y,z) like before; w only affects dynamics + norm
Xw = Xwrite(:,1:m_valid);
...
grid on; view(3)
title(sprintf('5D Hyper-Chen FB map(score\\_min) | time-shift IC | h=%.4f WT=%.3f Ky=%.1f', h, WT, Ky))
...
colorbar; colormap(turbo); caxis(CAX);
grid on; view(3)
title(sprintf('5D Hyper-Chen FB map(score\\_end) | time-shift IC | h=%.4f WT=%.3f Ky=%.1f', h, WT, Ky))

%% ================= SHOW TOP CONFIGS =================
[~,ord] = sort(results(:,5), 'ascend'); % by medMin
topN = min(15, size(results,1));
disp('Top configs sorted by medMin: [h WT Ky fracStrong medMin medEnd fracEndNeg]');
...
% 5th equation (swap if your 5D model differs)
% w' = -e*w + x*z
w = w + h1*( -e*w + x*z + N(5) );
