%% Hyper-Chen 5D+FB — FB map(score_end) with time-shift IC (z-only coupling)
clear; close all; clc
...
grid on; view(3)
title(sprintf('5D+FB FB map(score\\_end) in x-y-z | h=%.4f WT=%.3f Kz=%.1f | rho=%.2f e=%.2f | shift=%.2fs', ...
    h, WT, Kz, rho, e, t_shift));
