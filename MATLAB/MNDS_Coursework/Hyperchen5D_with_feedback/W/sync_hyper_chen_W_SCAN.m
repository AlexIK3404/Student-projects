%% Hyper-Chen 5D (with feedback) — FB phase map (w-only) + GRID SEARCH over (h, WT, Kw)
...
%% ================= GRID TO SEARCH (w-only coupling) =================
h_list  = [0.0025 0.005];
WT_list = [0.22 0.25 0.28 0.30 0.35 0.40 1.00];
Kw_list = [10 15 20 30 40 60 80 100 120 140];
...
%% ================= GRID SEARCH =================
best.metric = -Inf;
best.h = NaN; best.WT = NaN; best.Kw = NaN;
best.fracStrong = NaN; best.medMin = NaN;

results = []; % [h WT Kw fracStrong medMin medEnd fracEndNeg]

fprintf('Grid search: %d h * %d WT * %d Kw = %d configs\n', ...
    numel(h_list), numel(WT_list), numel(Kw_list), numel(h_list)*numel(WT_list)*numel(Kw_list));
...
for ih = 1:numel(h_list)
    ...
    for WT = WT_list
        ...
        for Kw = Kw_list
            ...
            results(end+1,:) = [h WT Kw fracStrong medMin medEnd fracEndNeg]; %#ok<SAGROW>

            metric = fracStrong*10 + (-medMin);
            if metric > best.metric
                best.metric = metric;
                best.h = h; best.WT = WT; best.Kw = Kw;
                best.fracStrong = fracStrong;
                best.medMin = medMin;
                fprintf('BEST: h=%.4f WT=%.3f Kw=%.1f | strong=%.2f medMin=%.2f medEnd=%.2f endNeg=%.2f\n', ...
                    h,WT,Kw,fracStrong,medMin,medEnd,fracEndNeg);
            end
        end
    end
end

fprintf('\nBest found: h=%.4f WT=%.3f Kw=%.1f | strong=%.2f medMin=%.2f\n', ...
    best.h, best.WT, best.Kw, best.fracStrong, best.medMin);
...
hw = waitbar(0,'Computing full phase map (5D time-shift IC)...');
...
grid on; view(3)
title(sprintf('5D FB map(score\\_end) | h=%.4f WT=%.3f Kw=%.1f | shift=%.2fs', ...
    h, WT, Kw, t_shift));
