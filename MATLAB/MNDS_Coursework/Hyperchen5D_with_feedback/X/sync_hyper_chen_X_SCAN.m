%% Hyper-Chen 5D (with feedback) — FB phase map (x-only) + GRID SEARCH over (h, WT, Kx)
...
Kx_list = [10 15 20 30 40 60 80 100 120 140];
    ...
    for WT = WT_list
        ...
        for Kx = Kx_list
            ...
            if metric > best.metric
                ...
                best.medMin = medMin;
                fprintf('BEST: h=%.4f WT=%.3f Kx=%.1f | strong=%.2f medMin=%.2f medEnd=%.2f endNeg=%.2f | rho=%.3f e=%.3f\n', ...
                    h,WT,Kx,fracStrong,medMin,medEnd,fracEndNeg,rho,e);
            end
        end
    end
end

fprintf('\nBest found: h=%.4f WT=%.3f Kx=%.1f | strong=%.2f medMin=%.2f | rho=%.3f e=%.3f\n', ...
    best.h, best.WT, best.Kx, best.fracStrong, best.medMin, rho, e);
...
grid on; view(3)
title(sprintf('5D+FB FB map(score\\_min) | time-shift IC | h=%.4f WT=%.3f Kx=%.1f | rho=%.3f e=%.3f', ...
    h, WT, Kx, rho, e))
...
colorbar; colormap(turbo); caxis(CAX);
grid on; view(3)
title(sprintf('5D+FB FB map(score\\_end) | time-shift IC | h=%.4f WT=%.3f Kx=%.1f | rho=%.3f e=%.3f', ...
    h, WT, Kx, rho, e))
...
% y implicit due to +c*y (include feedback with updated w)
