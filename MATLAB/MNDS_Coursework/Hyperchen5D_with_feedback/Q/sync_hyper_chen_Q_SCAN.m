%% Hyper-Chen 5D (with feedback) — FB phase map (q-only) + GRID SEARCH over (h, WT, Kq)
...
h_list  = [0.0025 0.005];
WT_list = [0.30 0.35 0.40 0.60 0.80 1.00];
Kq_list = [10 15 20 30 40 60 80 100 120 140];
...
grid on; view(3)
title(sprintf('5D FB map(score\\_min) | h=%.4f WT=%.3f Kq=%.1f | shift=%.2fs', ...
    h, WT, Kq, t_shift))
...
colorbar; colormap(turbo); caxis(CAX);
grid on; view(3)
title(sprintf('5D FB map(score\\_end) | h=%.4f WT=%.3f Kq=%.1f | shift=%.2fs', ...
    h, WT, Kq, t_shift))
