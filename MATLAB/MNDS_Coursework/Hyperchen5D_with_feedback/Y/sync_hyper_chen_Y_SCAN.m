%% Hyper-Chen 5D (with feedback) — FB phase map (y-only) + GRID SEARCH over (h, WT, Ky)
...
Ky_list = [10 15 20 30 40 60 80 100 120 140];
...
for ih = 1:numel(h_list)
    ...
    for WT = WT_list
        ...
        for Ky = Ky_list
            ...
            Kforward  = [0; Ky; 0; 0; 0];
            Kbackward = Kforward; % keep equal (как в базовом Чене)
            ...
            if metric > best.metric
                ...
                best.medMin = medMin;
                fprintf('BEST: h=%.4f WT=%.3f Ky=%.1f | strong=%.2f medMin=%.2f medEnd=%.2f endNeg=%.2f | rho=%.3f e=%.3f\n', ...
                    h,WT,Ky,fracStrong,medMin,medEnd,fracEndNeg,rho,e);
            end
        end
    end
end

fprintf('\nBest found: h=%.4f WT=%.3f Ky=%.1f | strong=%.2f medMin=%.2f | rho=%.3f e=%.3f\n', ...
    best.h, best.WT, best.Ky, best.fracStrong, best.medMin, rho, e);
...
grid on; view(3)
title(sprintf('5D+FB FB map(score\\_min) | time-shift IC | h=%.4f WT=%.3f Ky=%.1f | rho=%.3f e=%.3f', ...
    h, WT, Ky, rho, e))
...
colorbar; colormap(turbo); caxis(CAX);
grid on; view(3)
title(sprintf('5D+FB FB map(score\\_end) | time-shift IC | h=%.4f WT=%.3f Ky=%.1f | rho=%.3f e=%.3f', ...
    h, WT, Ky, rho, e))
...
% y implicit due to +c*y
% IMPORTANT: use feedback term rho*w here too (w is already stage-2 updated)
