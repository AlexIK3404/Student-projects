...
hw = waitbar(0,'Computing FB map (score_end) 5D (with feedback)...');
...
names  = {'x','y','z','q','w'};
pairs3 = nchoosek(1:5, 3);
pairs2 = nchoosek(1:5, 2);
...
if plot_all_3D
    for r = 1:size(pairs3,1)
        ...
        title(sprintf('5D+FB FB map(score\\_end) in %s-%s-%s | h=%.4f WT=%.3f Ky=%.1f | rho=%.2f e=%.2f | shift=%.2fs', ...
            names{i}, names{j}, names{k}, h, WT, Ky, rho, e, t_shift));
    end
else
    figure
    ...
    title(sprintf('5D+FB FB map(score\\_end) in x-y-z | h=%.4f WT=%.3f Ky=%.1f | rho=%.2f e=%.2f | shift=%.2fs', ...
        h, WT, Ky, rho, e, t_shift));
end
...
if plot_all_2D
    for r = 1:size(pairs2,1)
        ...
        title(sprintf('5D+FB FB map(score\\_end) in %s-%s | h=%.4f WT=%.3f Ky=%.1f | rho=%.2f e=%.2f | shift=%.2fs', ...
            names{i}, names{j}, h, WT, Ky, rho, e, t_shift));
    end
end
...
% y has +c*y -> implicit; include feedback rho*w here too (w already stage2-updated)
